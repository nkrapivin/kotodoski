...
# Данный сервер предназначен для хостинга на бесплатном PythonAnywhere.
...
def get_current_date() -> datetime:
    ...
    return datetime(dtnow.year, dtnow.month, dtnow.day)


def get_next_reset_date(reset_type: str) -> datetime:
    dtnow = get_current_date()

    if not reset_type:
        return None
    elif reset_type == 'day':
        return dtnow + timedelta(days=1)
    elif reset_type == 'week':
        return dtnow + timedelta(weeks=1)
    elif reset_type == 'hour':
        return dtnow + timedelta(hours=1)
...
def impl_post_leaderboard(user_id: str, user_name: str, leaderboard_id: str, metadata: str, score: int) -> tuple[bool, str]:
    ...
    if score is None or score <= 0:
        return (False, '{"status":-1,"error":"param score is invalid"}')

    if not user_id:
        return (False, '{"status":-2,"error":"param user_id is invalid"}')
    
    if not user_name:
        return (False, '{"status":-3,"error":"param user_name is invalid"}')
    
    if not leaderboard_id:
        return (False, '{"status":-4,"error":"param leaderboard_id is invalid"}')
    
    if not (leaderboard_id in data):
        return (False, '{"status":-5,"error":"leaderboard with given leaderboard_id does not exist"}')
    ...
    for idx in range(len(board_array)):
        ...
        if board_array[idx]['user_id'] == user_id:
            if data[leaderboard_id]['allow_overwrite']:
                ...
                return (False, '{"status":0,"error":"an entry for given user_id already exists"}')
    ...
    if our_index == -1:
        ...
        return (False, '{"status":-6,"error":"unable to find new entry index after sorting, WTF?!"}')
    # йиппи!!!!1
    return (True, '{"status":1,"error":"","new_entry_index":' + str(our_index) + '}')


def impl_json_escape_string(s: str) -> str:
    return \
        str(s) \
        .replace('\\', '\\\\') \
        .replace('\n', '\\n') \
...
def impl_get_leaderboard(user_id: str, leaderboard_id: str, index_start: int, amount: int) -> tuple[bool, str]:
    ...
    if not user_id:
        return (False, '{"status":-1,"error":"param user_id is invalid"}')

    if not leaderboard_id:
        return (False, '{"status":-2,"error":"param leaderboard_id is invalid"}')
    
    if (index_start is None) or (index_start < -1):
        return (False, '{"status":-3,"error":"param index_start is invalid"}')
    
    # index_start == -1 значит вывести относительно нашего пользователя
    
    if not (leaderboard_id in data):
        return (False, '{"status":-4,"error":"leaderboard with given leaderboard_id does not exist"}')
    
    # amount <= 0 значит вывести до конца списка, если возможно.
    if amount < 0:
        return (False, '{"status":-5,"error":"param amount is invalid"}')
    ...
    if index_start == -1:
        ...
        return (False, '{"status":0,"error":"index_start is -1 but player with specified user_id is not present"}')

    result = '{"status":1,"error":"","entries":['
    wrote = 0
    if amount == 0:
    ...
    for idx in range(index_start, index_start + amount):
        ...
        result += '{"score":' + str(entry['score'])
        result += ',"index":' + str(idx)
        result += ',"timestamp":' + str(entry['timestamp'])
        result += ',"user_id":"' + impl_json_escape_string(entry['user_id']) + '"'
        result += ',"user_name":"' + impl_json_escape_string(entry['user_name']) + '"'
        result += ',"metadata":"' + impl_json_escape_string(entry['metadata']) + '"'
        result += '}'
        wrote += 1

    result += '],"amount":' + str(wrote) + ',"total":' + str(entries) + '}'
    return (True, result)
...
def do_gas_request(gas_uid: str, gas_hash: str, gas_ip: str) -> tuple[bool, str]:
    ...
    with gas_lock:
        if not gas_uid:
            return (False, '{"status":-10,"error":"param gas_uid is invalid"}')
        
        if not gas_hash:
            return (False, '{"status":-11,"error":"param gas_hash is invalid"}')
        
        if not gas_ip:
            return (False, '{"status":-12,"error":"param gas_ip is invalid"}')
        
        gas_gmr_id = str(CONFIG_GAS_GMR_ID)
        if not gas_gmr_id:
            return (False, '{"status":-13,"error":"param gas_gmr_id is invalid"}')
        
        gas_secret = CONFIG_GAS_SECRET
        if not gas_secret:
            return (False, '{"status":-14,"error":"param gas_secret is invalid"}')
        ...
        if not gas_sign:
            return (False, '{"status":-15,"error":"failed to calculate gas_sign"}')
        ...
        try:
            ...
            if ok.status_code >= 400:
                return (False, '{"status":-16,"error":"gas api request forbidden"}')
            ok_json = ok.json()
            ok_json_status = ok_json['status']
            if ok_json_status != 'ok':
                return (False, '{"status":-17,"error":"gas api status failed"}')
            
            # TODO: пихать что-то полезнее чем 'ok'?
            gas_session_cache[cache_key] = ok_json_status
            return (True, cache_key)
        except:
            return (False, '{"status":-18,"error":"gas api request failed"}')
...
def do_vksteam_verify_ticket(ticket: str, user_id: str) -> tuple[bool, str]:
    with vksteam_lock:
        if not user_id:
            return (False, '{"status":-20,"error":"param user_id is invalid"}')
        
        if not ticket:
            return (False, '{"status":-21,"error":"param vksteam_ticket is invalid"}')
        ...
        try:
            ...
            return (False, '{"status":-25,"error":"vksteam api request failed"}')
...
def post_leaderboard():
    ...
    if CONFIG_USE_GAS:
        gas_uid = request.form.get('gas_uid')
        gas_hash = request.form.get('gas_hash')
        gas_ip = request.environ.get('HTTP_X_FORWARDED_FOR', request.remote_addr)
        gas_result = do_gas_request(gas_uid, gas_hash, gas_ip)
        if not gas_result[0]:
            return Response(response=gas_result[1], status=401, content_type='application/json; charset=utf-8')
        if not user_id:
            # разрешить не указывать user_id если он уже известен из GAS
            user_id = gas_uid
    elif CONFIG_USE_VKSTEAM:
        vksteam_ticket = request.form.get('vksteam_ticket')
        vksteam_result = do_vksteam_verify_ticket(vksteam_ticket, user_id)
        if not vksteam_result[0]:
            return Response(response=vksteam_result[1], status=401, content_type='application/json; charset=utf-8')
...
def get_leaderboard():
    ...
    if CONFIG_USE_GAS:
        gas_uid = request.args.get('gas_uid')
        gas_hash = request.args.get('gas_hash')
        gas_ip = request.environ.get('HTTP_X_FORWARDED_FOR', request.remote_addr)
        gas_result = do_gas_request(gas_uid, gas_hash, gas_ip)
        if not gas_result[0]:
            return Response(response=gas_result[1], status=401, content_type='application/json; charset=utf-8')
        ...
        vksteam_ticket = request.args.get('vksteam_ticket')
        vksteam_result = do_vksteam_verify_ticket(vksteam_ticket, user_id)
        if not vksteam_result[0]:
            return Response(response=vksteam_result[1], status=401, content_type='application/json; charset=utf-8')
